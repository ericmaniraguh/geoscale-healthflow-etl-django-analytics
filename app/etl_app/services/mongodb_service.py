# etl_app/services/postgresql_service.py - FIXED ANALYTICS TABLE CREATION
...
class PostgreSQLService:
    ...
    def save_raw_data(self, data: List[Dict[str, Any]], table_name: str = "health_center_lab_data", 
                     update_mode: str = 'replace') -> Tuple[bool, str]:
        """Save raw data with robust transaction handling"""
        if not data:
        ...
        logger.info(f"STARTING: Save {len(data)} records to '{table_name}' (mode: {update_mode})")
        ...
        try:
            with self.engine.begin() as conn:
                ...
                for i, record in enumerate(data):
                    try:
                        safe_record = {
                            ...
                            'created_at': record.get('created_at') or datetime.now().strftime('%Y-%m-%dT%H:%M:%S')
                ...
                logger.info(f"SUCCESS: {table_name} now contains {final_count} total records")
                
                message = f"Successfully processed {len(data)} records to '{table_name}'. Final count: {final_count}"
                if update_mode == 'append':
    ...
    def save_analytics(self, analytics_data: Dict[str, Any], table_prefix: str = "analytics",
        ...
        """FIXED: Save all analytics with proper error handling and data conversion"""
        if not self.engine:
            return False, {"error": "Cannot connect to PostgreSQL"}
        
        logger.info(f"ANALYTICS SAVE: Starting with prefix '{table_prefix}'")
            ...
            with self.engine.begin() as conn:
                ...
                for analytics_type, data in analytics_data.items():
                    ...
                    try:
                        ...
                        # Generate table name and schema type
                        if analytics_type == 'yearly_slide_status':
                            table_name = generate_dynamic_table_name(f"{table_prefix}_yearly_statistics", district, sector, years)
                            schema_type = 'yearly_stats'
                        elif analytics_type == 'gender_positivity_by_year':
                            table_name = generate_dynamic_table_name(f"{table_prefix}_gender_pos_by_year", district, sector, years)
                            schema_type = 'gender_pos'
                        elif analytics_type == 'village_positivity_by_year':
                            table_name = generate_dynamic_table_name(f"{table_prefix}_village_pos_by_year", district, sector, years)
                            schema_type = 'village_pos'
                        elif analytics_type == 'total_summary':
                            table_name = generate_dynamic_table_name(f"{table_prefix}_total_summary", district, sector, years)
                            schema_type = 'summary'
                        elif analytics_type == 'monthly_positivity':
                            table_name = generate_dynamic_table_name(f"{table_prefix}_monthly_positivity", district, sector, years)
                        ...
                        logger.info(f"ANALYTICS: Table name for {analytics_type}: {table_name}")
                        
                        # Save this analytics table with improved error handling
                        success = self._save_analytics_table_fixed(
                            conn, table_name, data, schema_type, update_mode, 
                            district, sector, years, timestamp, analytics_type
                        )
                ...
                logger.info("ANALYTICS SAVE: All analytics processed in single transaction")
                logger.info(f"ANALYTICS RESULTS: {results}")
                return True, results

        except Exception as e:
            logger.error(f"ANALYTICS SAVE TRANSACTION ERROR: {str(e)}")
            logger.error(f"ANALYTICS SAVE TRACEBACK: {traceback.format_exc()}")
    ...
    def _clean_value_for_db(self, value, field_name: str):
        ...
        if isinstance(value, (list, dict)):
            return json.dumps(value) if field_name in ['years_covered', 'districts_covered', 'sectors_covered', 'gender_breakdown', 'age_group_breakdown'] else str(value)
